...
from utils import normalize_id, interpret_table_output, upload_s3_object, CLOUDFRONT_DOMAIN
...
if os.getenv("LOCAL"):
    BASE_DIR = Path("/Users/rondineli/datasets/amateur").resolve()
    DEFAULT_DATASET_DIR = "/Users/rondineli/datasets/amateur/"
    DPATH_JSON_DATASET = os.path.join("/Users/rondineli/datasets/amateur/", "download")
...
DOWNLOAD_JOBS_FILE = "/tmp/jobs.json"
...
def make_download_post_hook(job_id: Optional[bool] = False):
    def download_post_hook(status: str) -> None:
        ...
        final_file_download_name = f"{DEFAULT_DATASET_DIR}download/{raw_video_id}/{raw_video_id}.mp4"
        file_download_name = status.get("info_dict", {}).get("filename", final_file_download_name)

        print(f"file_download_name => {file_download_name}")

        print(f"final_file_download_name => {final_file_download_name}")

        parsed_path_final = Path(file_download_name)

        print(f"parsed_path_final.name: {parsed_path_final.name}")
        ...
        s3_key = f"videos/downloads/{parsed_path_final.name}"

        if status.get("status") == "finished" and job_id:
            db["status"] = status_download
            upload_s3_object(
                file_object=db["file_download_name"],
                file_name=s3_key
            )
            db["s3_location"] = f"{CLOUDFRONT_DOMAIN}{s3_key}"
            # db_1 = db["file_download_name"]
            print(f"Uploaded {db['s3_location']} to s3")

        db["file_location"] = final_file_download_name
        db["file_download_name"] = file_download_name
    ...
    return download_post_hook


def make_download_ph_hook(job_id: Optional[bool] = False):
    """ wrapper method to encapsulate the decorated function """
    def download_ph_hook(status: dict) -> None:
        ...
        """
        print(f"[DEBUG] Job id started: {job_id}")
        status_download = status.get("status")
        raw_video_id = status.get("info_dict", {}).get("id")
        id_file = raw_video_id
        id_file = os.path.join(DBASE_PATH_JSON, normalize_id(id_file))
        ...
        if job_id:
            file_download_name = status.get("filename")

        percent_str = status.get("_percent", "0")
        ...
        db = {
            ...
            "status_completion": round(percent, 2),
    ...
    return download_ph_hook


def start_download(url: str, download_only: Optional[bool] = False) -> None:
    """
    Step 1 of main handler: Method to start youtubeVideo Id download from youtube API
    """

    out_tmp = f"{DEFAULT_DATASET_DIR}download/%(id)s/%(id)s.%(ext)s"

    if download_only:
        out_tmp = f"{DEFAULT_DATASET_DIR}download/%(id)s/%(title)s [%(id)s].%(ext)s"
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ydl.add_progress_hook(make_download_ph_hook(download_only))
        ydl.add_postprocessor_hook(make_download_post_hook(download_only))
        ydl.download([url])
...
def load_jobs():
    if not os.path.exists(DOWNLOAD_JOBS_FILE):
        return {}
    with open(DOWNLOAD_JOBS_FILE, "r") as f:
        return json.load(f)
...
# youtube endpoint download only
...
def create_download():
    ...
    if not data or "youtubeUrlOrId" not in data:
        return jsonify({"error": "Missing youtubeUrlOrId"}), 400

    youtube_url_or_id = data["youtubeUrlOrId"]

    if youtube_url_or_id.startswith("http"):
        youtube_id = youtube_url_or_id.split("v=")[1]
    else:
        youtube_id = youtube_url_or_id

    print(f"youtube_id => {youtube_id}")


    id_file = os.path.join(DBASE_PATH_JSON, normalize_id(youtube_id))
    id_file = f"{id_file}.json"
    ...
    return jsonify({"jobId": youtube_id, "url": f"/status/{youtube_id}"})
